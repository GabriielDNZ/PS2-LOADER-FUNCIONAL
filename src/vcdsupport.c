...
// Auto-detect the appropriate BDMA MODE for the given SOURCE device.
// Probes for the exFAT variant files (usbd.irx.<suffix> + usbhdfsd.irx.<suffix>) in the
// source device's POPS/ folder. Returns the matching VCD_BDMA_* mode if found, or
// VCD_BDMA_FAT32 if no variant files are present (device is FAT32 or files are missing).
int vcdAutoDetectBdmaMode(int source)
{
    if (source < 0 || source >= VCD_BDMA_SRC_COUNT)
        return VCD_BDMA_FAT32;

    // Map source -> its exFAT mode (the one variant that matches the source device family)
    static const int srcToMode[VCD_BDMA_SRC_COUNT] = {
        VCD_BDMA_USBEXFAT, // SRC_USB   -> usbexfat
        VCD_BDMA_MX4SIO,   // SRC_MX4SIO -> mx4sio
        VCD_BDMA_MMCE,     // SRC_MMCE  -> mmce
        VCD_BDMA_ATA,      // SRC_HDD   -> ata
    };
    int mode = srcToMode[source];
    const char *suffix = vcdBdmaSuffix[mode];

    // Resolve source device root (same logic as vcdEquipBdma)
    const char *cands[2];
    ...
    if (source == VCD_BDMA_SRC_MMCE) {
        ...
        int wantType = (source == VCD_BDMA_SRC_MX4SIO) ? BDM_TYPE_SDC :
                       (source == VCD_BDMA_SRC_HDD)    ? BDM_TYPE_ATA : BDM_TYPE_USB;
    ...
    LOG("[BDMA] auto-detect: no exFAT variant files for source %d, falling back to FAT32\n", source);
    return VCD_BDMA_FAT32;
}
