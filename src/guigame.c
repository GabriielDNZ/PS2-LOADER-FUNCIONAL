...
static int guiGameSavePadEmuGameConfig(config_set_t *configSet, int result)
{
    if (gPadEmuSource == SETTINGS_PERGAME) {
        ...
        // Always write ENABLEPADEMU and PADEMUSETTINGS when per-game source is active,
        // even when the values are 0 (explicitly disabled / default settings).
        // Previously these keys were removed when 0, causing them to be reloaded as
        // "not found" on the next open, which reset EnablePadEmu to 0 silently.
...
static void guiGameLoadPadEmuConfig(config_set_t *configSet, config_set_t *configGame)
{
    ...
    // Override with per-game settings if selected.
    // If the per-game keys are missing (old config or never saved), keep the
    // global values instead of resetting to 0 (which caused the "resets on open" bug).
    if (!forceGlobalPadEmu) {
        configGetInt(configSet, CONFIG_ITEM_PADEMUSOURCE, &gPadEmuSource);
        if (gPadEmuSource == SETTINGS_PERGAME) {
            // configGetInt returns non-zero on success; only override if the key exists
            int tmpEnable = EnablePadEmu;
            int tmpSettings = PadEmuSettings;
            if (configGetInt(configSet, CONFIG_ITEM_ENABLEPADEMU, &tmpEnable))
                EnablePadEmu = tmpEnable;
            if (configGetInt(configSet, CONFIG_ITEM_PADEMUSETTINGS, &tmpSettings))
                PadEmuSettings = tmpSettings;
        }
    }
    // set gui settings.
