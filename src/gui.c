...
void guiShowConfig()
{
    ...
    // BDMA (BDMAssault exFAT driver) equip. MODE is auto-detected from SOURCE (no manual selection).
...
reshow_config:
    ...
    if (ret) {
        ...
        {
            // Auto-detect the BDMA MODE from the SOURCE device: probe for exFAT variant files in
            // its POPS/ folder. If found -> equip exFAT; if not -> equip FAT32 (built-in driver).
            // Only fires when SOURCE actually changed OR no equip has been done yet (MODE = FAT32
            // but SOURCE device now has exFAT files available).
            int oldSrc = gBdmaSource;
            int newSrc = oldSrc;
            diaGetInt(diaConfig, CFG_BDMASOURCE, &newSrc);
            int autoMode = vcdAutoDetectBdmaMode(newSrc);
            if (newSrc != oldSrc || autoMode != gBdmaMode) {
                char bdmaDiag[160];
                int er = vcdEquipBdma(newSrc, autoMode, bdmaDiag, sizeof(bdmaDiag));
                if (er == -4) {
                    char msg[256];
                ...
                gBdmaMode = vcdReadBdmaMode(); // sync to what's actually on the card
            }
            gBdmaSource = newSrc;
        }
