...
#ifdef DS5_ENABLE
// DS5 player indicator LEDs (bits 0-3 map to the 4 touchpad LEDs)
static u8 ds5_player_leds[] = {0x01, 0x03, 0x07, 0x0F};
// DS5 output report ID for USB
...
#ifdef DS5_ENABLE
    if (device->idVendor == DS34_VID && device->idProduct == DS5_PID)
        return 1;
#endif
...
static void readReport(u8 *data, int pad_idx)
{
    ...
    if (data[0]) {
        ...
        if (pad->type == DS3) {
            ...
            translate_pad_ds5(report, &pad->ds2);
            padMacroPerform(&pad->ds2, report->PSButton);

            if (report->PSButton) {                                    // display battery level
                if (report->Create && (pad->btn_delay == MAX_DELAY)) { // PS + Create (= Select)
                    if (pad->analog_btn < 2)                           // unlocked mode
                        pad->analog_btn = !pad->analog_btn;
                    ...
                    pad->oldled[0] = report->Battery * 25; // 0-10 -> 0-255
                    pad->oldled[1] = 0;
                    pad->oldled[2] = 0;
            ...
            // charging = Power bit indicates charging state (bit 4 = charging)
            if (report->Usb_plugged && report->Power != 0x02) // 0x02 = fully charged
                pad->oldled[3] = 1;
            else
                pad->oldled[3] = 0;
#endif
...
static int LEDRumble(u8 *led, u8 lrum, u8 rrum, int pad)
{
    ...
    if (ds34pad[pad].type == DS3) {
        ...
        // DS5 USB output report 0x02
        usb_buf[0] = DS5_USB_OUTPUT_REPORT_ID;
        usb_buf[1] = 0xFF; // valid_flag0: rumble + LED
        usb_buf[2] = 0xF7; // valid_flag1: lightbar + player LEDs
        usb_buf[3] = rrum; // right (weak) rumble
        usb_buf[4] = lrum; // left (strong) rumble

        // Bytes 5-8: reserved / headphone/mic flags
        usb_buf[9] = led[0];  // lightbar R
        usb_buf[10] = led[1]; // lightbar G
        usb_buf[11] = led[2]; // lightbar B

        if (led[3]) {           // charging blink
            usb_buf[42] = 0x02; // player LED fade (pulse)
        } else {
            usb_buf[42] = 0x00;
        }
        usb_buf[43] = ds5_player_leds[pad & 3]; // player indicator 1-4
