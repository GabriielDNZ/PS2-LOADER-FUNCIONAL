...
#ifdef DS5_ENABLE
                    } else if (memcmp(hci_buf + 9, "DualSense Wireless Controller", 29) == 0) {
                        ds34pad[i].type = DS5;
                        ds34pad[i].isfake = 0;
                        DPRINTF("\t Type: DualSense (PS5) \n");
#endif
                    } else {
                        ds34pad[i].type = DS3;
...
static int hid_initDS34(int pad)
{
    ...
    if (ds34pad[pad].type == DS3) {
        ...
        // DS5 BT init: request feature report 0x05 to enable full HID mode
        init_buf[0] = HID_THDR_GET_REPORT_FEATURE; // THdr
        init_buf[1] = 0x05;                        // DS5 feature report ID
#endif
    }
...
// DS5 BT output report 0x31 (49 bytes payload)
#define DS5_BT_REPORT_LEN 74
        ...
        led_buf[11] = led[0]; // lightbar R
        led_buf[12] = led[1]; // lightbar G
        led_buf[13] = led[2]; // lightbar B

        if (led[3]) {           // charging blink
            led_buf[44] = 0x02; // player LED pulse
        }
        led_buf[45] = ds5_player_leds[pad & 3]; // player indicator

        size += DS5_BT_REPORT_LEN;
#endif
    }
        ...
        if (data[9] == PS3_01_REPORT_ID) {
...
#ifdef DS5_ENABLE
        } else if (data[9] == 0x31) { // DS5 BT input report ID
            // DS5 BT report: bytes 0-7 = L2CAP header, byte 8 = THDR, byte 9 = report ID
            // Actual DS5 data starts at byte 11 (same offset as DS4)
            struct ds5report *report = (struct ds5report *)&data[11];
            ...
            if (report->PSButton) {
                if (report->Create && (pad->btn_delay == MAX_DELAY)) { // PS + Create
                    ...
                    pad->oldled[0] = report->Battery * 25;
                    pad->oldled[1] = 0;
                    pad->oldled[2] = 0;
            ...
            if (report->Usb_plugged && report->Power != 0x02)
                pad->oldled[3] = 1;
            else
                pad->oldled[3] = 0;
#endif
