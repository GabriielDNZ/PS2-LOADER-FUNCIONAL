...
#ifdef DS5_ENABLE
void translate_pad_ds5(const struct ds5report *in, struct ds2report *out)
{
    // DS5 dpad uses the same hat-switch encoding as DS4
    static const u8 dpad_mapping[] = {
        (DS2ButtonUp),
        (DS2ButtonUp | DS2ButtonRight),
        (DS2ButtonRight),
    ...
    // Create = Select equivalent, Option = Start equivalent
    // Touchpad click splits into Select/Start by finger position (same as DS4)
    u8 select = in->Create;
    u8 start = in->Option;
